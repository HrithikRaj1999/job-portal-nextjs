...
const OptionGenerate = ({ from, ...rest }: { from: string[]; [x: string]: any }) => (
  <>
...
const JobFilterSideBar = async ({ filterValues: { search, type, location, remote } }: { filterValues: JobFilterTypes }) => {
  ...
  return (
    <aside className="sticky top-0 h-fit rounded-lg border bg-background">
      <form action={filterJobs} key={JSON.stringify({ search, type, location, remote })}>
        <div className="flex flex-col ">
          <div className="flex flex-col gap-3 p-4">
            <Label htmlFor="search" className="text-md">
              Search
            </Label>
            <Input defaultValue={search} id="search" name="search" placeholder="Title, company, etc." />
          </div>
          <div className="flex flex-col gap-3 p-4">
          ...
          <div className="flex items-center gap-3 p-4">
            <input id="remote" type="checkbox" name="remote" defaultChecked={remote} className="scale-125 accent-slate-900" />
